...
- **トップページに波の背景を追加**: SVGとCSSによる軽量なアニメーションを追加しました。スマートフォンでは波を減らし、ダークモードと端末の「動きを減らす」設定にも対応しています
- **GitHubへのリンクを追加**: トップページ下部からソースコードを新しいタブで開けるようになりました
- **初期表示を高速化**: 画面ごとの遅延読み込みとスケジュール取得の集約により、起動時の読み込みを軽量化しました
- **シフト保存・復元の整合性を改善**: 同時更新の競合、日別保存、バックアップ復元、大量登録への対応を強化し、取得失敗時の誤操作を防止しました
- **希望休と設定データの整合性を改善**: 希望休の月次重複を防ぎ、スタッフ・クラス削除、並び替え、設定更新と操作履歴の整合性を改善しました
- **ログインと入力検証を強化**: ログインのレート制限を追加し、管理API・CSVインポートの入力検証と認可を強化しました
- **オフラインのExcel出力に対応**: PWAのキャッシュ導入後、初回のExcel出力をオフラインでも利用できるようにしました
- **保守性と品質確認を改善**: API・設定・スタッフ管理の構成を整理し、統合テストとE2Eテストを拡充、依存関係を更新しました
