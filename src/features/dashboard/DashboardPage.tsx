...
import { AlertTriangle, BarChart3, CalendarDays, CheckCircle2, Clock3, Users } from 'lucide-react';
...
export default function DashboardPage() {
    ...
    const cards = [
        { label: '予定勤務時間', value: `${formatHours(analysis.summary.scheduledHours)}h`, icon: Clock3, color: 'text-indigo-600' },
    ...
    return (
        <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 md:p-8">
            <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                ...
                <input type="month" value={yearMonth} onChange={event => setYearMonth(event.target.value)} className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-slate-800 dark:border-slate-600 dark:bg-slate-800 dark:text-white" aria-label="対象月" />
            </header>
            ...
            {!loading && !error && <>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{cards.map(({ label, value, icon: Icon, color }) => <div key={label} className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800"><Icon className={`h-5 w-5 ${color}`} /><p className="mt-3 text-xs font-bold text-slate-500">{label}</p><p className="mt-1 text-2xl font-bold text-slate-900 dark:text-white">{value}</p></div>)}</div>
