...
import { useShiftRequirements } from '../../../lib/hooks';
import type { ShiftRequirement, ShiftClass } from '../../../types';
...
interface ShiftRequirementsSectionProps {
    classes: ShiftClass[];
    classesLoading: boolean;
}

const ShiftRequirementsSection = ({ classes, classesLoading }: ShiftRequirementsSectionProps) => {
    ...
    const addTimeSlot = () => {
        if (!selectedClass) { toast.error('クラスを選択してください'); return; }
        const newReq = createEmptyRequirement();
        newReq.classId = selectedClass;
    ...
    const handleDeleteConfirm = () => {
        if (deleteTargetId) setRequirements(requirements.filter(r => r.id !== deleteTargetId));
    ...
    const hasOverlappingSlots = (): boolean => {
        ...
        for (let i = 0; i < sorted.length - 1; i++) {
            ...
            if (current.classId === next.classId && current.dayOfWeek === next.dayOfWeek) {
                if (current.endTime > next.startTime && current.startTime < next.endTime) return true;
            }
        }
        return false;
    };
    ...
    const filteredRequirements = selectedClass ? requirements.filter(r => r.classId === selectedClass) : [];

    if (loading) {
        return (
            <div className="p-12 text-center text-slate-400 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            {selectedClass && (
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                        <h3 className="text-lg font-semibold text-slate-800 dark:text-white">時間帯設定</h3>
                        <span className="text-sm text-slate-500 dark:text-slate-400">{filteredRequirements.length} 件の設定</span>
                    </div>
                                ...
                                <div className="divide-y divide-slate-200 dark:divide-slate-700">
                                    {filteredRequirements.map((req, index) => (
                                        <SortableRequirementRow key={req.id} req={req} index={index}
                                            onUpdate={updateRequirement} onDeleteRequest={handleDeleteRequest} />
                                    ))}
                                </div>
                            </SortableContext>
                        </DndContext>
                    ...
                    <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                        <button onClick={addTimeSlot} disabled={!selectedClass}
            ...
            {selectedClass && (
                <div className="flex justify-end gap-3 pt-4">
                    ...
                    <button onClick={handleSave} disabled={saving || !isDirty}
                        ...
                        {saving ? (<><Loader2 className="w-5 h-5 animate-spin" /><span>保存中...</span></>) : (<span>保存</span>)}
                    </button>
                </div>
            )}
...
export default ShiftRequirementsSection;
