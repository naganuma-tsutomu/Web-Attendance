...
const DailyTimelineModal: React.FC<DailyTimelineModalProps> = ({
    ...
    const [initialShifts] = useState(localShifts);
    ...
    const handleMouseMove = useCallback((e: MouseEvent) => {
        ...
        if (drag.type === 'move') {
            ...
            const groups: (ClassType | 'unassigned')[] = ['スマイル組', '虹組', '特殊', 'unassigned'];
    ...
    const handleTimeInputBlur = async () => {
        // 自動保存を廃止したため、ここでは何もしない
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const modifiedIds = Object.keys(localShifts).filter(id => {
                if (id.startsWith('temp-')) return false; // 新規分は別途処理済み
                const current = localShifts[id];
                const initial = initialShifts[id];
                return initial && (
                    current.start !== initial.start ||
        ...
        >
            ...
            >
                ...
                <div className="flex-1 overflow-auto p-6 min-h-0">
                        ...
                        <div className="bg-white dark:bg-slate-800 pb-2">
                            {['スマイル組', '虹組', '特殊', 'unassigned'].map(groupClass => {
                                ...
                                return (
                                    ...
                                    >
                                        {/* Group Header */}
                                        <div className={`px-4 py-2 text-sm font-bold border-t border-b transition-colors duration-200 flex items-center justify-between ${hoveredGroup === groupClass ? 'ring-2 ring-inset ring-indigo-500 bg-indigo-50 dark:bg-indigo-900/30' : titleColor
                                            }`}>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex w-[330px] flex-shrink-0 text-sm bg-white dark:bg-slate-800">
                                                            {/* 名前セクション */}
                                                            <div className="w-28 p-2 border-r border-slate-200 dark:border-slate-700 flex flex-col justify-center overflow-hidden relative group/name">
                                                                <div className="font-medium text-slate-800 dark:text-slate-200 truncate" title={staffName}>
                                                                    {staffName}
                                                                </div>
                                                                <button
                                                                    onClick={() => handleRemoveShift(shift.id)}
                                                                    className="absolute right-1 top-1/2 -translate-y-1/2 p-1 text-slate-300 hover:text-red-500 opacity-0 group-hover/name:opacity-100 transition-all hover:bg-red-50 dark:hover:bg-red-900/30 rounded"
                                                                    title="この日のシフトから削除"
                                                                >
                                                        ...
                                                        >
                                                            ...
                                                            <div
                                                                className={`absolute top-2 bottom-2 rounded border shadow flex items-center
                                                                        ${getBarColor(isDragging && hoveredGroup ? hoveredGroup : s.classType, s.isError)}
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700/50 flex flex-wrap items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
                        ...
                        <div className="ml-auto flex items-center gap-4">
                            {isModified ? (
                                <>
                                    <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 animate-in fade-in slide-in-from-right-2 duration-300">
                                        <Save className="w-4 h-4" />
                                        <span className="font-medium text-[11px]">変更後は「保存する」ボタンで確定してください</span>
                                    </div>
                                ...
                                <div className="h-9" /> /* ボタンが表示されていない時も高さを確保 */
                            )}
                        </div>
                    </div>
                </div>
